...
\begin{center}
{\Large \scshape Mathematical Proofs and Mathematical Writing}

\textit{Ebrahim Ebrahim}
\end{center}
...
marked ``term.''
%\begin{center}\includegraphics[scale=0.8]{img/subst.pdf}\end{center}
Some examples:
\indented{
...
Appendex \ref{appendix:thms} can be a useful reference throughout.
...
In
the following theorem, we will use ``$\tA$''  as a placeholder for a \emph{term},
just as we use ``$\pA$'' as a placeholder for a \emph{proposition}.
We will use the notation ``$\SUBST{\tA}{\boxsubscript{30}{variable}}{\boxsubscript{20}{term}}$'' to indicate
that a substitution is to be made in a term, just as we have been using ``$\SUBST{\pA}{\boxsubscript{30}{variable}}{\boxsubscript{20}{term}}$''
to indicate
that a substitution is to be made in a proposition.
We can apply parts
\ref{itm:eq_used:2}
and
\ref{itm:eq_used:4}
...
\THMNP{Equality Used\label{thm:eq_used}}{
If $x=y$, then:
\begin{enumerate}[(1)]
...
But it turns out to be broken in the worst possible way-- Axiom \ref{ax:in} allows us to prove a contradiction
(see appendix \ref{sec:bugappendix}).
...
This is what we do throughout this text, while just bearing in mind that
there are some propositions $\pA$ on which Axiom \ref{ax:in} should not be used.
There are comments in appendix \ref{sec:bugappendix} on how to fix the bug;
most of our approach is not affected much by the fix. 
...
\begin{samepage}
Before ending this section, we take a moment to point out that membership is normally not transitive;
...
\begin{center}
\hfill\includegraphics[scale=1]{img/beans/beans4.pdf} \hfill \raisebox{2em}{\tagA}
\end{center}
Then $g\circ f$ can be determined to have the picture
\begin{center}
\hfill\includegraphics[scale=1]{img/beans/beans5.pdf} \hfill \raisebox{2em}{\tagB}
\end{center}
which is $\{(y,b),(z,d)\}$.
...
\THMNP{Bijectivity and Isomorphism\label{thm:bij_and_iso}}{Assume
that $f:X\rightarrow Y$.
...
\DEFN{Order for $\N$\label{def:order_for_N}}{
\begin{enumerate}[(1)]
...
\ex{\label{ex:mult_intro}Assume that $a\in\N$.
...
\begin{center}\includegraphics[scale=1]{img/embed.pdf}\end{center}
...
\DEFN{\label{def:finiteness}Finiteness}{
\begin{enumerate}[(1)]
...
In the next definition, we will group some of these adjectives together in ways that have proven useful in practice.
We will primarily be concerned with \emph{equivalence relations},
but we also highlight some of the other types of relations that people often care about.
...
So an equivalence relation (on $A$) is a relation (on $A$) that is reflexive (on $A$), transitive, and symmetric.
When $\REL$ is an equivlanece relation on $A$, we say often read ``$a\REL b$'' as ``$a$ and $b$ are \emph{equivalent}.''
...
\item $\REL\,\,=\COLL{(n,n+1)}{n\in\N}$
\item $\REL\,\,=\COLL{(n,n+1)}{n\in\N} \cup \COLL{(n+1,n)}{n\in\N}$
\item $\REL\,\,=\COLL{(n,m)}{n,m\in\N\ \text{ and $n+m$ is even}}$\ \ (recall Definition \ref{defn:even})
\end{enumerate}
}

\ex{\label{ex:powerset_relns}
\begin{enumerate}
...
\item\label{ex:powerset_relns:eq}
Define $\REL\,\,=\COLL{(X,Y)}{X,Y\subseteq A\ \AND\ X\eqnm Y}$.
...
where everything in any given batch is connected to everything else in the same batch, and where there are no connections between different batches.
...
\ex{Define $\REL\,\,=\COLL{(X,Y)}{X,Y\subseteq A\ \AND\ X\eqnm Y}$.
You proved in Exercise
\ref{ex:powerset_relns}
part
\ref{ex:powerset_relns:eq}
that $\REL$ is an equivalence relation on $\SUB{A}$.
Now suppose that $x\in A$.
Describe the equivalence classes $\CLASS{\emptyset}{\REL}$ and $\CLASS{\{x\}}{\REL}$.
(Hint: revisit the ideas around Definition \ref{def:finiteness}.)
}

\ex{Show that the following $\REL$ is an equivalence relation on $A\times B$, and then describe the equivalence classes:
$$
\REL\,\,=\COLL{((a,b),(a',b))}{a,a'\in A\ \  \AND\ \ b\in B}.
$$
}

\DEFN{Set of Equivalence Classes}{Assume
that $\REL$ is an equivalence relation on $A$. Then we define
$$
\MODREL{A} \ =\ \COLL{\CLASS{a}{\REL}}{a\in A}.
$$
}

Referring back to the intuition from Exercise \ref{ex:eqblobs},
an equivalence relation on a set \emph{partitions} the set into batches called equivalence classes.
We can make this concept precise. First, we make precise the concept of a \emph{partition} of a set:

\def\part{\mathcal{F}}
\DEFN{Partition\label{def:partition}}{A set $\part\subseteq\SUB{A}$ of subsets
of $A$ is a \emph{partition of $A$} iff
\begin{enumerate}[(1)]
  \item \label{itm:partdef:1}$\emptyset\notin\part$,
  \item \label{itm:partdef:2}$\ALL{a}{a\in A\Rightarrow \EXIST{F}{F\in\part\ \ \AND\ \ a\in F}}$, and
  \item \label{itm:partdef:3}$\ALL{F,F'}{F,F'\in\part\ \ \Rightarrow\ \ (F\neq F'\ \Rightarrow\ F\cap F'=\emptyset)}$.
\end{enumerate}
}

Condition \ref{itm:partdef:2} says that the sets in $\part$ must ``cover'' all of $A$ and
condition \ref{itm:partdef:3} says that the sets in $\part$ must be pairwise disjoint.
Condition \ref{itm:partdef:1} is an unimportant technicality that makes the
upcoming Theorem \ref{thm:corr_parts_eqrelns} have a pretty statement.
Next, we look at how equivalence relations give rise to partitions and vice versa:

\ex{\label{ex:eqreln_to_part}Suppose that $\REL$ is an equivalence relation on $A$. Prove that $\MODREL{A}$ is a partition of $A$.}

\ex{\label{ex:part_to_eqreln}Let $\part$ be a partition of $A$. Prove that
$$ \COLL{(a,b)}{\EXIST{F}{F\in\part\ \AND\ a,b\in F}} $$
...
% \def\corr{A/\,\begin{picture}(-1,1)(-1,-3)\circle*{3}\end{picture}\,\,}
\def\corr{f}
\def\corrinv{g}
\def\eqrelnsA{\mathcal{E}}
\def\partsA{\mathcal{P}}
\THMNP{Correspondence between Equivalence Relations and Partitions\label{thm:corr_parts_eqrelns}}{
Given a set $A$, define
\begin{align*}
\eqrelnsA\ &=\ \COLL{\REL}{\REL \text{is an equivalence relation on $A$}}\\
\partsA\ &=\ \COLL{\part}{\part\ \text{is a partition of $A$}}
\end{align*}
and define a function $\corr$ on $\eqrelnsA$ by
$$
\corr(\REL) = \MODREL{A} \hspace{1.5em} \text{for all $\REL\,\in\eqrelnsA$}.
$$
Then
$
\corr:
...
Exercise \ref{ex:eqreln_to_part} proves that $\ran{\corr}\subseteq\partsA$,
so we have $\corr:\eqrelnsA\to\partsA$. It remains to prove that this mapping is bijective.
We will use Theorem \ref{thm:bij_and_iso} to do this. Define a function $\corrinv$ on $\partsA$ by
$$\corrinv(\part) = \COLL{(a,b)}{\EXIST{F}{F\in\part\ \AND\ a,b\in F}} $$
for all $\part\in\partsA$.
Exercise \ref{ex:part_to_eqreln} proves that $\ran{\corrinv}\subseteq\eqrelnsA$,
so we have $\corrinv:\partsA\to\eqrelnsA$.

\textit{Proof that $\corrinv\circ\corr =\id{\eqrelnsA}$:}
Consider any $\REL\,\in\eqrelnsA$. We will show that $\corrinv(\corr(\REL))=\,\REL$.

For the inclusion $\corrinv(\corr(\REL))\subseteq\,\REL$, consider any $(a,b)\in \corrinv(\corr(\REL))$.
There is some $F\in\corr(\REL)$ such that $a,b\in F$.
Since $F\in\corr(\REL)=\MODREL{A}$, there is some $c\in A$ such that $F=\CLASS{c}{\REL}$.
Since $a,b\in F=\CLASS{c}{\REL}$, we have $a\REL c$ and $b\REL c$.
It follows that $a\REL b$, since $\REL$ is an equivalence relation. In other words, $(a,b)\in\,\REL$.
...
Okay, now stop cheating.
We define an integer to be an \emph{equivalence class} of pairs of natural numbers,
where the equivalence relation being used is
$$
(n,m)\REL(k,l) \ \Leftrightarrow\ m+k=l+n.
$$
More formally:

\THMPL{The following relation on the set $\N\times\N$ is an equivalence relation:
$$ \COLL{((n,m),(k,l))}{n,m,k,l\in\N\ \ \AND\ \ m+k=l+n}. $$
}{\putExerciseHeading.}{\label{thm:integers_equiv_reln}}

\DEFN{Integers}{Let $\REL$ be the equivalence relation on $\N\times\N$ defined in Theorem \ref{thm:integers_equiv_reln}.
We define
$$ \Z = \MODREL{(\N\times\N)}.$$
}

We end here.
There's a lot more work to be done to give the integers $\Z$ the structure that makes them behave like integers.
We only defined addition (Definition \ref{def:plus}) for natural numbers--
work is needed to see how $\Z$ can inherit the addition operation.
Work is also needed to see how ordering for $\N$ (Definition \ref{def:order_for_N}) can be inherited by $\Z$.

Exercise \ref{ex:mult_intro} hints at how one could use recursion to define multiplication on $\N$.
Further work can pass along the multiplication operation to $\Z$.
Just as the need for subtraction led us to construct $\Z$ out of $\N$, a need for division can lead one 
to construct the rational numbers, $\Q$, out of $\Z$.
Eventually you can branch out to specific fields like abstract algebra or real analysis.
...
\section{The Bug}\label{sec:bugappendix}

Axiom \ref{ax:in} makes it possible to prove a contradiction, 
and this breaks our entire system (recall Derived Rule \ref{rule:contradictions}).
This bug was first encountered in 1897 when Burali-Forti unknowingly proved a theorem
that directly contradicted an earlier theorem of Cantor.
It caused quite a panic when Bertrand Russel pointed it out a few years later.
The original path to the bug was complicated,
but now thanks to Russell there is a much simpler way to
see how a contradiction arises from Axiom \ref{ax:in}.

\def\russ{\mathcal{R}}

Define a set $\russ = \COLL{x}{x\notin x}$.
Now ask yourself: Is $\russ\in\russ$?
By Axiom \ref{ax:in} we have
$$
\russ\in\russ\ \ \Leftrightarrow \ \ \SUBST{\text{``\ $x\notin x$\ ''}}{x}{\russ}.
$$
In other words,
\[
\russ\in\russ\ \ \Leftrightarrow \ \ \russ\notin\russ
\label{eqn:russ}\tag{$\ast$}
\]
Due to excluded middle, we have $(\russ\in\russ)\OR(\russ\notin\russ)$.
In the case that $\russ\in\russ$ we get $\russ\notin\russ$ from (\ref{eqn:russ}).
In the case that $\russ\notin\russ$ we get $\russ\in\russ$ from (\ref{eqn:russ}).
This completes a proof by cases in which we have proven the contradiction
$$
(\russ\in\russ)\AND(\russ\notin\russ).
$$
What a disaster! The framework in this text is completely broken.
That's why it's called \emph{naive set theory}.

After the panic, people started coming up with different ways to fix the bug.
Each approach led to a certain kind of set theory. Here we will briefly look at two
popular flavors of set theory, NBG and ZF. Both fixes of the bug come down to the same basic idea:
restrict who gets to be on the left hand side of ``$\in$''.

\subsection{NBG}

The NBG approach (Von Neumann–Bernays–Gödel set theory) is to replace Axiom \ref{ax:in} by the following:

\newcommand{\insets}[1]{\EXIST{b}{#1\in b}}
\def\ainsets{\insets{a}}

\AXIOMNMOCK{Membership, fixed version}{
$$a\in\COLL{x}{\pA}\DARR\ (\SUBST{\pA}{x}{a}\ \AND\ \ainsets).$$
}

This makes the proposition ``$a\in\COLL{x}{\pA}$'' harder to prove-- one
has to establish an extra condition $\ainsets$.
In other words: For $a$ to be a member of $\COLL{x}{\pA}$, 
in addition to satisfying the membership condition $\pA$
it has to also be a known member of \emph{something}.

At first glance this might not sound so hard. After all, isn't any given $a$ at least
a member of $\{a\}$? Not so!
From Definition \ref{def:singleton},
$$\{a\} = \COLL{x}{x=a},$$
so by the new version of Axiom \ref{ax:in} we have
$$
a\in \{a\} \ \DARR\ (a=a \ \AND\ \ainsets).
$$
By Axiom \ref{ax:refl_eq} we can deduce
$$
a\in \{a\} \ \DARR\ \ainsets,
$$
but we are unable to deduce $a\in\{a\}$!

The extra condition ``$\ainsets$'' in the new Axiom \ref{ax:in} blocks us
from ever proving the elementhood of $a$ in anything.
If we just knew that $a$ is a member of \emph{something} then $\ainsets$ becomes true
and the new Axiom \ref{ax:in} reduces to the old Axiom \ref{ax:in} for ``$a\in\ldots$'' statements.

Some bootstrapping is needed to get our first ``'$a\in\ldots$'' statements.
This comes in the form of a whole bunch of new Axioms in NBG such as

\AXIOMMOCK{$\insets{\emptyset}$.}

This forcibly admits $\emptyset$ into the exclusive club of $a$'s for which $\ainsets$.
Being in this club means that ``$\emptyset\in\ldots$'' statements effectively get to follow the rules of the old Axiom \ref{ax:in}.
For example, this makes it possible to use Defintion \ref{def:singleton} to prove that $\emptyset\in\{\emptyset\}$.
But we still can't prove $\{\emptyset\}\in\{\{\emptyset\}\}$-- for that we might need another axiom like the following:

\AXIOMMOCK{$\insets{a}\ \ARR\ \insets{\{a\}}$.}

NBG has many more axioms like these to make set theoretic constructions such as doubleton, union, powerset, etc. work properly
with the new Axiom \ref{ax:in}.

How does this repair the contradiciton in (\ref{eqn:russ}) above?
With the new Axiom \ref{ax:in}, we replace (\ref{eqn:russ}) by
$$
\russ\in\russ\ \ \Leftrightarrow \ \ (\russ\notin\russ\ \AND\ \insets{\russ}).
$$
Assuming $\russ\in\russ$ still leads to a contradiction, so we must have $\russ\notin\russ$.
Now assuming $\insets{\russ}$ leads back to $\russ\in\russ$ and another contradiction, so we must have
$$
\neg \insets{\russ}.
$$
Instead of giving a contradiction when we ask whether $\russ\in\russ$, the new Axiom \ref{ax:in}
simply tells us that $\russ$ never gets to be a member of \emph{anything}.

In NBG set theory, the word \emph{set} is reserved only for those $a$'s for which $\ainsets$.
Otherwise the more general term \emph{class} is used instead.
What we've established above is that $\russ$ is a class that is not a set.

\subsection{ZF}

ZF (Zermelo-Fraenkel) set theory is another very popular way to deal with the paradoxes of naive set theory.
Instead of starting with a fix to Axiom \ref{ax:in}, ZF set theory treats the notation ``$\COLL{x}{\pA}$''
in  a completely different way.

When we write ``$\COLL{x}{\pA}$,'' our intent is to refer to
$$
\text{``a set $S$ with the property that $a\in S\DARR\SUBST{\pA}{x}{a}$ for all $a$''}.
$$
ZF set theory wants us to stop and ask ourselves: Does such a set \emph{exist}?
In  other words:
$$
\EXIST{S}{\ALL{a}{a\in S \DARR \SUBST{\pA}{x}{a}}}?
$$
If yes, then a ZF axiom like our Axiom \ref{ax:eq_used} can be used to prove that such an $S$ is unique.
In ZF set theory we think of the notation ``$\COLL{x}{\pA}$'' as a shorthand for
$$
\text{``the unique set $S$ with the property that $a\in S\DARR\SUBST{\pA}{x}{a}$ for all $a$''}.
$$

Just like with NBG, some bootstrapping is needed in ZF to get set theoretic constructions like doubleton, union, powerset, etc. to work properly.
This comes in the form of a bunch of axioms about the \emph{existence} of various sets.

How does ZF help with the problematic set $\russ$ at the beginning of this appendix?
Well, due to the contradiction in (\ref{eqn:russ}) we conclude that there \emph{does not exist}
a set $\russ$ for which $x\in\russ\DARR x\notin x$ for all $x$.
If you write ``$\COLL{x}{x\notin x}$'' then you are trying to refer to
$$
\text{``the unique set $\russ$ for which $x\in\russ\DARR x\notin x$ for all $x$''}.
$$
This is as nonsensical as trying to refer to ``the unique element of the empty set.''
One can write the words, but they fail to refer to anything.
...
\section{Theorem Reference}\label{appendix:thms}
