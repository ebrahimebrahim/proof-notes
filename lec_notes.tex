...
These adjectives are grouped together in ways that have proven useful in practice, as shown in the following definition.
...
Referring back to the intuitiion from Exercise \ref{ex:eqblobs},
an equivalence relation on a set \emph{partitions} the set into batches called equivalence classes.
We can make this concept precise:

\def\part{\mathcal{F}}
\DEFN{Partition}{A set $\part\subseteq\SUB{A}$ of subsets
of $A$ is a \emph{partition of $A$} iff
\begin{enumerate}[(1)]
  \item $\ALL{a}{a\in A\Rightarrow \EXIST{F}{F\in\part\ \ \AND\ \ a\in F}}$ and
  \item $\ALL{F,F'}{F,F'\in\part\ \ \Rightarrow\ \ (F\neq F'\ \Rightarrow\ F\cap F'=\emptyset)}$.
\end{enumerate}
}

In fact, equivalence relations and partitions are two ways of looking at the same information:

\def\corr{A/\,\begin{picture}(-1,1)(-1,-3)\circle*{3}\end{picture}\,\,}
\THMNP{Correspondence between Equivalence Relations and Partitions}{
Given a set $A$, define
$$ \corr\ =\ \COLL{(\REL\ ,\ \MODREL{A})}{\REL \text{is an equivalence relation on $A$}}. $$
Then
$$
\corr:
\COLL{\REL}{\REL \text{is an equivalence relation on $A$}}
\to
\COLL{\part}{\part\ \text{is a partition of $A$}}
$$
and this is a bijection.
}{
  \note{Proof coming soon}
}

\note{Ex labeled F coming soon}
...
Our approach will be the following strategy which is common throughout mathematics:
...
\note{Discuss doing this in the case of integers}

\note{Def: integers are \MODREL{\N\times \N}}

\note{Comments on work to be done: 
Addition. Ordering.
Multiplication for \N from Ex, then also multiplicaton for \Z. 
Rationals, using mult and similar ideas. Also transfer ordering.
Reals, focusing on partial ordering.}
