...
It is therefore useful to have a formal mechanism for ``plucking'' the single occupant of a signleton out from between the curly braces.
...
We will not use Axiom \ref{ax:choice}, but it is absolutely essential for much of modern mathematics.
...
\ex{Prove that $x<\next{y}\ \DARR\ x\leq y$. We use this fact freely below.}

\THMN{About $0$\label{thm:aboutzero}}{
If $n\in\N$, then
\begin{enumerate}[(1)]
...
\proofnewline
\textit{Proof of \ref{thm:aboutzero1}:}
We will prove that $0<\next{n}$ for all natural numbers $n$, by doing induction on $n$.

\BChead{}
We have $0<\next{0}$ because we obviously have $0\in0\cup\{0\}$.

\IShead{}
Suppose that $n\in\N$ and $0<\next{n}$.
We must now show that $0<\next{\next{n}}$.
Since 
$$\next{\next{n}}=\next{n}\cup\{\next{n}\},$$
we have $\next{n}\subseteq\next{\next{n}}$.
Therefore, since we know that $0\in \next{n}$, we have $0\in\next{\next{n}}$.
That is, $0<\next{\next{n}}$.

By induction, we have proven that $0<\next{n}$ for every natural number $n$.
\done
\proofsep
\textit{Proof of \ref{thm:aboutzero2}:}
Assume that $n\in\N$. Observe that
$$ 0\leq n\  \DARR\  (0\in n\,\OR\,0=n)\ \DARR\ (n\neq 0\ARR0\in n),$$
so we may prove $n\neq 0\ARR0\in n$.
Assume that $n\neq 0$.
By Theorem \ref{thm:pred}, we can get an $m\in\N$ such that $n=\next{m}$.
By part \ref{thm:aboutzero1} of the present theorem, we have $0\in\next{m}=n$.
\done
\proofsep
\textit{Proof of \ref{thm:aboutzero3}:}
Assume that $n\in\N$.
...
\THMN{Succession Preserves Order\label{thm:succ_ord}}{Assume that $n\in\N$.
\begin{enumerate}[(1)]
...
\proofnewline
\textit{Proof of \ref{thm:succ_ord1}:}
We will prove
$$m<n\ \ARR\ \next{m}\leq n$$
...
Our assumption $m\in\next{n}=n\cup\{n\}$ implies that either $m\in n$ or $m=n$.

Case 1: Suppose that $m\in n$. Then $\next{m}\leq n$ by the induction hypothesis (\ref{eqn:succ_ord}).
So either $\next{m}\in n$ or $\next{m}=n$.
...
\textit{Proof of \ref{thm:succ_ord2}:}
Assume that $n\in\N$ and $m<n$.
Then $\next{m}\leq n$, by part \ref{thm:succ_ord1}.
Either $\next{m}\in n$, or $\next{m}=n$. Therefore $\next{m}\in n\cup\{n\}=\next{n}$.
\done
...
Since $m\in k\cup\{k\}$, we have either $m\in k$ or $m = k$.

Case 1: Suppose that $m\in k$.
...
It follows that $n\in k\cup\{k\} = \next{k}$. In other words, $n<\next{k}$.
This completes the induction step.
}

\THMPL{If $n\in m$ and $m\in\N$, then $n\in\N$.}{
\putExerciseHeading.
Hint: Do an induction on $m$ to prove ``$n\in m\ARR n\in\N$'' for all $m\in\N$.
}{\label{thm:N_closed}}
...
Then $\next{n}\in n\cup\{n\}$, so we have either $\next{n}\in n$ or $\next{n}=n$.
