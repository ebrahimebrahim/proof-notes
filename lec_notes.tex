...
Case 1: Suppose that $z=\emptyset$. Then $z\subseteq \{x\}$ (Theorem \ref{thm:emptyset_minimal}\ref{thm:emptyset_minimal1}).

Case 2: Suppose that $z=\{x\}$. Then $z\subseteq \{x\}$ (Theorem \ref{thm:refl_trans_incl}\ref{thm:refl_trans_incl1}).
...
If $f$ is a function and $x\in\dom{f}$,
then
\begin{enumerate}[(1)]
\item \label{thm:ess_eval1} $(x,f(x))\in f$.
...
\THMP{Assume that $f:X\rightarrow Y$.
...
We clearly have $x,z\in\dom{f}=X$, and so we may apply Theorem \ref{thm:ess_eval}
...
To show that $f$ is injective, consider any $x,y\in X$ such that $f(x)=f(y)$.
...
\THMP{Assume that $f:X\rightarrow Y$. Then $f$ is surjective onto $Y$ if and only if
$$
...
\subsection{Composition, Reversal, and Image}
